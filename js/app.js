...
async function loadMyOrders() {
  const el = document.getElementById(
    currentUser ? 'myOrdersList' : 'guestOrdersList'
  );
  ...
  try {
    ...
    if (currentUser) {
      ...
      const ids = JSON.parse(
        localStorage.getItem('myOrderIds') || '[]'
      );

      const snaps = await Promise.all(
        ids.map(id => getDoc(doc(db, 'orders', id)))
      );
    ...
    if (!docs.length) {
      el.innerHTML = `
        <div class="account-empty" style="padding:18px;">
          <div class="em">📦</div>
          <p>مفيش طلبات لسه</p>
        </div>
      `;
    ...
    const card = x => {
      ...
      const price =
        x.total != null
          ? `${x.grandTotal != null ? x.grandTotal : x.total} جنيه`
          : 'في انتظار التسعير';
      ...
      return `
        <div class="my-order-card">
          ...
          <button
            class="add-addr-btn"
            onclick="window.openOrderInvoice('${x.id}')">
...
window.openOrderInvoice = id => {
  window.location.href = orderInvoiceUrl(id);
};
...
function renderProducts() {
  ...
  grid.innerHTML = list.map(p => {
    ...
    const oldPrice =
      p.discount
        ? `<div class="product-old">${p.price} جنيه</div>`
        : '';
    ...
    return `
      <div class="product-card ${!inStock ? 'out-of-stock' : ''}">
        ...
        <div
          class="product-img"
          style="${p.imageUrl ? 'background:#1a2940;' : ''}">
          ...
          ${
            !inStock
              ? '<div class="out-badge">نفد المخزون</div>'
              : ''
          }
...
function renderSavedAddrPicker() {
  ...
  list.innerHTML =
    myAddresses.map(a => `
      ...
      <div
        class="saved-addr-pick ${a.id === selectedAddrId ? 'sel' : ''}"
        onclick="window.pickAddr('${a.id}')">
    ...
    `
      <div
        class="saved-addr-pick ${selectedAddrId === null ? 'sel' : ''}"
        onclick="window.pickAddr(null)">

        <b>
          ✍️ عنوان تاني (اكتبه تحت)
        </b>
...
function translateAuthErr(e) {
  ...
  if (
    ...
    return 'البريد ده متسجل قبل كده';
  ...
  if (
    ...
    return 'البريد الإلكتروني مش صحيح';
...
function renderAddresses() {
  ...
  wrap.innerHTML =
    myAddresses.map(a => `
      ...
      <div
        class="addr-card ${a.isDefault ? 'default' : ''}">
        ...
        <div class="addr-label">
          ...
          ${
            a.isDefault
              ? '<span class="addr-default-chip">أساسي</span>'
              : ''
          }
        ...
        <div class="addr-actions">
          ...
          ${
            !a.isDefault
              ? `
                <button
                  onclick="window.makeDefault('${a.id}')">
          ...
          <button
            class="danger"
            onclick="window.deleteAddr('${a.id}')">
...
window.saveAddressForm = async () => {
  ...
  if (!data.city || !data.street)
    return toast(
      '⚠️ اكتب على الأقل المدينة والشارع'
    );
...
window.submitRxOrder = async () => {
  ...
  if (!rxBase64)
    return toast(
      '⚠️ ارفع صورة الروشتة الأول'
    );


  toast(
    '⏳ جاري حفظ الطلب وإرساله للصيدلي...'
  );
  ...
  try {
    ...
    const blob =
      await (
        await fetch(rxBase64)
      ).blob();


    const fileName =
      `prescriptions/${Date.now()}-${Math.random().toString(36).slice(2)}.${mimeType.split('/')[1] || 'jpg'}`;
    ...
    toast(
      '🎉 تم استلام الروشتة! تقدر تفتح طلبك من «طلباتي» وتتابع السعر والحالة.'
    );
    ...
    toast(
      '❌ حصل خطأ أثناء حفظ الروشتة، حاول تاني'
    );
