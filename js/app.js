import { firebaseConfig, SCAN_FUNCTION_URL } from './firebase-config.js';
...
window.placeOrder = async () => {
  ...
  try {
    ...
    setTimeout(() => {
      window.location.href = 'https://flourishing-klepon-a6460c.netlify.app/?order=' + ref.id;
    }, 1200);
...
window.updateRxMed = (i, val) => { rxMeds[i] = { ...rxMeds[i], name: val }; };
...
window.submitRxOrder = async () => {
  ...
  try {
    ...
    try {
      const path = `prescriptions/${Date.now()}_${Math.random().toString(36).slice(2)}.jpg`;
      const sref = ref(storage, path);
      await uploadString(sref, rxBase64, 'data_url');
      prescriptionImageUrl = await getDownloadURL(sref);
    } catch(upErr) {
    ...
    const docRef = await addDoc(collection(db, 'orders'), {
      ...
      userId: currentUser ? currentUser.uid : null,
      customer: { name, phone, address },
      items: rxMeds.map(m => ({ name: m.name, qty: 1, price: 0, emoji: '💊' })),
    ...
    closeRx();
    clearRx();
    toast('🎉 تم إرسال الروشتة! الصيدلي هيتواصل معاك يأكد السعر');
    setTimeout(() => {
      window.location.href = 'https://flourishing-klepon-a6460c.netlify.app/?order=' + docRef.id;
    }, 1500);
  } catch(e) {
    console.error(e);
    toast('❌ حصل خطأ، حاول تاني');
