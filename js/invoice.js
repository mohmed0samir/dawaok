...
function showNotFound() {
  el.innerHTML = `
    <div class="center-state">
      ...
      <button class="btn btn-teal" onclick="location.href='/'" style="margin:auto;">🏪 روح للمتجر</button>
    </div>`;
}
...
function renderInvoice(data) {
  ...
  const delivery   = isRx ? (Number(data.deliveryFee) || 0) : 25;
  ...
  const itemsHTML = (data.items || []).map(item => {
    ...
    const priceCell = isRx ? 'يحدده الصيدلي' : `${item.price} ج`;
    const totalCell = isRx ? '—' : `${item.price * item.qty} ج`;
    return `<tr>
  ...
  const rxPending = isRx && (data.status || 'pending') === 'pending';
  const rxConfirmed = isRx && (data.status || '') === 'confirmed';

  const bannerHTML = isRx ? `
    <div class="success-banner rx">
      <div class="success-icon">${rxPending ? '⏳' : rxConfirmed ? '✅' : '🧾'}</div>
      <div class="success-text">
        <h3>${rxPending ? 'تم استلام روشتتك بنجاح!' : rxConfirmed ? 'تمت مراجعة الروشتة وتحديد السعر' : 'تحديث على طلب الروشتة'}</h3>
        <p>${rxPending
          ? 'الصيدلي بيراجع الروشتة دلوقتي. الصفحة دي هتتحدث تلقائياً لما يحدد الأدوية والأسعار.'
          : rxConfirmed
            ? 'تم تحديد الأدوية والأسعار. لو مناسبك، تابع مع الصيدلية لتأكيد التوصيل.'
            : 'تقدر تتابع حالة طلبك من نفس الصفحة.'}</p>
      </div>
    </div>` : `
  ...
  const totalsHTML = isRx ? `
    <div class="inv-totals">
      <div class="inv-total-row"><span class="lbl">إجمالي الأدوية</span><span class="amt">${subtotal ? subtotal + ' جنيه' : 'في انتظار التسعير'}</span></div>
      <div class="inv-total-row"><span class="lbl">رسوم التوصيل</span><span class="amt">${data.status === 'confirmed' ? delivery + ' جنيه' : 'تحدد عند التأكيد'}</span></div>
      <div class="inv-total-row grand"><span class="lbl">الإجمالي الكلي</span><span class="amt">${data.status === 'confirmed' ? (grandTotal + ' جنيه') : 'في انتظار مراجعة الصيدلي'}</span></div>
    </div>` : `
    <div class="inv-totals">
