  ...
  try {
    ...
    const res = await fetch(
      'https://us-central1-pharmacy-b198d.cloudfunctions.net/sendPrescriptionToTelegram',
      {
        ...
        body: JSON.stringify({ name, phone, address, imageBase64, mimeType })
      }
    );

    const result = await res.json();
    if (!res.ok || !result.success) throw new Error(result.error || 'Telegram error');

    closeRx();
    clearRx();
    toast('🎉 تم إرسال الروشتة للصيدلي بنجاح');
  } catch (e) {
    console.error(e);
