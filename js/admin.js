...
/* ─── قراءة المنتج من الصورة — عن طريق Cloud Function (مش مباشرة من المتصفح) ───
   السبب: مفتاح Anthropic API لازم يفضل سري على السيرفر، مينفعش يتحط في كود
   المتصفح لأي حد يقدر يشوفه. الحل: Firebase Cloud Function بتحمل المفتاح
   كـ secret وتنادي Claude من هناك. راجع functions/index.js + README.md */
async function scanImage(base64){
  const badge=document.getElementById('scanBadge');
  const nameEl=document.getElementById('pName');
  const brandEl=document.getElementById('pBrand');
  badge.className='scan-badge scanning';
  badge.style.display='block';
  badge.textContent='🔍 بنقرأ الصورة بالذكاء الاصطناعي...';
  nameEl.disabled=true; brandEl.disabled=true;
  try{
    const b64=base64.split(',')[1];
    const mime=base64.split(';')[0].split(':')[1]||'image/jpeg';
    const res=await fetch(window.SCAN_FUNCTION_URL,{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({ mode:'product', imageBase64:b64, mimeType:mime })
    });
    if(!res.ok) throw new Error('server '+res.status);
    const r=await res.json();
    if(r.name){
      nameEl.value=r.name;
      brandEl.value=r.brand||'';
      const cats=['أدوية','فيتامينات','عناية بالبشرة','صحة الطفل','مستلزمات طبية','تكميل عضلات','عروض'];
      const m=cats.find(c=>(r.category||'').includes(c));
      if(m) document.getElementById('pCat').value=m;
      badge.className='scan-badge done';
      badge.textContent='✅ تم التعرف — عدّل لو محتاج';
    } else throw new Error('no name');
  } catch(e){
    console.error(e);
    badge.className='scan-badge err';
    badge.textContent='⚠️ مقدرناش نقرأ الصورة أوتوماتيك — اكتب الاسم يدوياً (تأكد من نشر Cloud Function)';
  }
  nameEl.disabled=false; brandEl.disabled=false;
}
...
function renderOrders(){
  ...
  tb.innerHTML=o.map(x=>{
    ...
    return `<tr>
      ...
      <td>${chip(x.status)}</td>
      <td>
        ${x.status==='pending'?`<button class="abtn ok" onclick="updOrder('${x.docId}','confirmed')">✅ تأكيد</button>`:''}
...
function viewInv(docId){
  window.open('https://flourishing-klepon-a6460c.netlify.app/?order='+docId,'_blank');
}
