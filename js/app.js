...
let selectedLocation = { cart:null, rx:null, address:null };
let mapTarget = null;
...
window.pickAddr = (id) => {
  ...
  if (id) {
    ...
    selectedLocation.cart = a.location || null;
  } else {
    document.getElementById('custAddress').value = '';
    selectedLocation.cart = null;
  }
  updateLocationSummary('cart');
};
...
function prefillCheckoutFromProfile() {
  ...
  if (def) { selectedAddrId = def.id; document.getElementById('custAddress').value = formatAddr(def); selectedLocation.cart = def.location || null; updateLocationSummary('cart'); }
}
...
window.placeOrder = async () => {
  ...
  if (!currentUser) { toast('⚠️ سجل دخولك أولًا لإتمام الطلب وحمايته'); return; }
  ...
  try {
    const ref = await addDoc(collection(db, 'orders'), {
      ...
      customer: { name, phone, address, location: selectedLocation.cart },
      items: cart,
      total,
      status: 'pending',
...
window.editAddr = (id) => {
  ...
  if (!a) return;
  editingAddrId = id;
  selectedLocation.address = a.location || null;
  updateLocationSummary('address');
...
window.openRx = () => {
  ...
  if (currentUser) {
    ...
    if (def?.location) { selectedLocation.rx = def.location; updateLocationSummary('rx'); }
  }
};
window.closeRx = () => {
...
// ── Location capture: no Maps API key is placed in the frontend. The saved
// coordinates are attached to the address/order and opened by the courier in Google Maps.
function updateLocationSummary(target) {
  const loc = selectedLocation[target];
  const el = document.getElementById(target + 'LocationSummary');
  if (!el) return;
  el.textContent = loc ? `✓ تم حفظ الموقع: ${Number(loc.lat).toFixed(5)}, ${Number(loc.lng).toFixed(5)}` : 'لم يتم تحديد موقع على الخريطة بعد';
}

window.openMapPicker = target => {
  mapTarget = target;
  const loc = selectedLocation[target];
  document.getElementById('mapModal').classList.add('open');
  document.getElementById('mapPreview').textContent = loc ? `الموقع المحفوظ: ${Number(loc.lat).toFixed(5)}, ${Number(loc.lng).toFixed(5)}` : 'اضغط «استخدام موقعي الحالي» للسماح بالوصول للموقع.';
  const link = document.getElementById('mapExternalLink');
  if (loc) {
    link.href = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(loc.lat + ',' + loc.lng)}`;
    link.style.display = 'block';
  } else link.style.display = 'none';
};
window.closeMapPicker = () => document.getElementById('mapModal').classList.remove('open');
window.useCurrentLocation = () => {
  if (!mapTarget || !navigator.geolocation) return toast('⚠️ المتصفح لا يدعم تحديد الموقع');
  const preview = document.getElementById('mapPreview');
  preview.textContent = '⏳ جاري تحديد موقعك...';
  navigator.geolocation.getCurrentPosition(pos => {
    const loc = { lat: pos.coords.latitude, lng: pos.coords.longitude, capturedAt: new Date().toISOString() };
    selectedLocation[mapTarget] = loc;
    preview.textContent = `✓ تم حفظ الموقع بدقة تقريبية ${Math.round(pos.coords.accuracy)} متر`;
    const link = document.getElementById('mapExternalLink');
    link.href = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(loc.lat + ',' + loc.lng)}`;
    link.style.display = 'block';
    updateLocationSummary(mapTarget);
  }, () => {
    preview.textContent = 'تعذر الوصول للموقع. تأكد من السماح بإذن الموقع ثم حاول مرة أخرى.';
...
window.submitRxOrder = async () => {
  ...
  if (!currentUser) return toast('⚠️ سجل دخولك أولًا لإرسال الروشتة وحمايتها');
  ...
  try {
    ...
    const orderRef = await addDoc(collection(db, 'orders'), {
      ...
      type: 'prescription',
      userId: currentUser ? currentUser.uid : null,
      customer: { name, phone, address, location: selectedLocation.rx },
      items: [],
      total: null,
      deliveryFee: 25,
      grandTotal: null,
