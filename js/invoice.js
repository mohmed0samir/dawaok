...
function showNotFound() {
  el.innerHTML = `
    <div class="center-state">
      ...
      <button class="btn btn-teal" onclick="location.href='./index.html'" style="margin:auto;">🏪 روح للمتجر</button>
    </div>`;
}

function statusInfo(s) {
  return {
    pending:   ['طلب جديد / قيد المراجعة', 'status-pending'],
    priced:    ['تم التسعير', 'status-confirmed'],
    ready:     ['جاهز للتوصيل', 'status-confirmed'],
    assigned:  ['تم تعيين مندوب', 'status-confirmed'],
    picked_up: ['استلم المندوب الطلب', 'status-confirmed'],
    out_for_delivery: ['جاري التوصيل', 'status-pending'],
